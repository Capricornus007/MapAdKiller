...
public final class SettingsTile extends TileService {
    ...
    /**
     * 磁貼亮滅要跟設定頁讀同一把鍵：模塊自己的 RemotePreferences。
     * 上游那套 App.hideIcon() 走的是進程內 SharedPreferences，與 hook/設置側的
     * RemotePreferences 是兩個地方，並存會讓磁貼狀態永遠顯示錯。
     */
    private static boolean hiddenHere() {
        try {
            return Config.prefs().getBoolean(Config.K_HIDE_ICON, false);
        } catch (Throwable t) {
            return false;
        }
    }
    ...
    public void onStartListening() {
        try {
            ...
            if (t != null) {
                ...
                t.setState(hiddenHere() ? Tile.STATE_INACTIVE : Tile.STATE_ACTIVE);
                t.updateTile();
            }
